...
observer = Observer()
observer.add_display(imageSize=[400, 640, 1], scalingFactor=2, name='Downward Facing Image')
observer.add_display(imageSize=[480, 640, 3], scalingFactor=2, name='RealSense RGB Image')
observer.add_display(imageSize=[480, 640, 1], scalingFactor=2, name='RealSense Depth Image')
observer.add_plot(numMeasurements=1680, frameSize=400, pixelsPerMeter=50, scalingFactor=4, name='Leishen Lidar')
observer.launch()
"""


def launch_embedded_observer():
    """Start the Observer UI in a fresh Python interpreter using the embedded script."""
    cmd = [sys.executable, "-u", "-c", EMBEDDED_OBSERVER_SCRIPT]
    return subprocess.Popen(cmd)

class Keyboard():
    """Interface for receiving teleoperation commands from the keyboard agent."""

    def __init__(self,ip='localhost'):
        """Initialize buffers and open a non-blocking stream to the keyboard server."""
    ...
    def status_check(self, message, iterations=10):
        """Block until the stream connects or the iteration limit is hit."""
    ...
    def read(self):
        """Poll the keyboard stream and refresh cached command values."""
        new = False
    ...
    def terminate(self):
        """Close the underlying stream cleanly."""
...
class Camera3D():
    """Helper for configuring and reading RGB-D camera streams over Video3D."""
    ...
    def __enter__(self):
        """Return self so instances can be used with context managers."""
        return self

    def __exit__(self, type, value, traceback):
        """Ensure streams are stopped when leaving a with-block."""
        self.terminate()

class Camera2D():
    """Manage configuration and acquisition for standard 2D cameras."""
    ...
    def __enter__(self):
        """Return self so callers can use the camera in a with-statement."""
        return self

    def __exit__(self, type, value, traceback):
        """Terminate the camera when leaving a with-statement."""
        self.terminate()
...
class Probe():
    ...
    def __init__(self,
                 ip = 'localhost'):
        """Initialize agent registries for the remote probe endpoints."""
    ...
    def add_display(self,
        ...
        """Register a streaming display endpoint on the Observer host."""
    ...
    def add_plot(self,
        ...
        """Register a polar plot endpoint for LiDAR data."""
        self.numPlots += 1
    ...
    def add_scope(self,
        ...
        """Register a scope endpoint for time-series data."""
        self.numScopes += 1
    ...
    def terminate(self):
        """Close every active streaming agent."""
        for key in self.agents:
...
class RemotePlot: # works as a client
    """Client helper for streaming polar LiDAR plots to the Observer UI."""
    ...
    def __init__(
        ...
        """Prepare a streaming client that will publish polar plot images."""
        self.scalingFactor = scalingFactor
        self.scaling = scaling
        self.numMeasurements = numMeasurements
        if scaling:
    ...
    def send(self, distances = None, angles = None):
        """Transmit the supplied ranges/angles if the plot socket is open."""
...
class RemoteScope():
    """Client streamer for sending sampled time-series data to Observer."""
    ...
    def __init__(
        ...
        """Configure the underlying stream used to push scope samples."""
    ...
    def send(self, time, data = None):
        """Send a timestamped sample vector to the remote scope."""
        if data is None:
            return False
...
class QBotPlatformDriver():
    ...
    def __init__(self, mode=1, ip='192.168.2.15') -> None:
        """Create a TCP client that exchanges basic IO packets with the driver."""
    ...
    def status_check(self, message, iterations=10):
        """Keep polling until the driver connection is available or timeout hits."""
        self._timeout = Timeout(seconds=0, nanoseconds=1000) #1000000
        counter = 0
        while not self._handle.connected:
            self._handle.checkConnection(timeout=self._timeout)
    ...
    def read_write_std(self,
                       ...
                       color=[1, 0, 1],
                       hold = 0):
        """Exchange a single control packet and update cached telemetry."""
    ...
    def terminate(self):
        """Shutdown the driver stream cleanly."""
        self._handle.terminate()
...
class QBotPlatformLidar(Lidar):
    ...
    def __init__(
        ...
        """Initialize the LiDAR wrapper with platform-specific connection URIs."""
...
class QBotPlatformRealSense(Camera3D):
    ...
    def __init__(
        ...
        """Select the correct RealSense endpoint and forward settings to Camera3D."""
...
class QBotPlatformCSICamera(Camera2D):
    ...
    def __init__(
        ...
        """Pick the proper CSI camera URI and forward parameters to Camera2D."""
...
try:
    ...
    observer_proc = None
    print(f"Warning: Unable to auto-start Observer ({exc}). Please launch manually if needed.")
...
startTime = time.time()
def elapsed_time():
    """Return the number of seconds that have passed since setup."""
...
try:
    ...
    try:
        yolo_model = YOLO(r"D:\Quanser\Quanser\runs\detect\train5\weights\best.pt")
        print("YOLO model loaded. RealSense RGB stream will show detections.")
    except Exception as exc:
        yolo_model = None
        print(f"Warning: YOLO model unavailable ({exc}). Proceeding without detections.")
    frame_shape = realSenseCam.imageBufferRGB.shape
    last_rgb_viz = np.zeros(frame_shape, dtype=np.uint8)
    yolo_enabled = True
        ...
        if probe.connected:
            ...
            if newHIL:
                ...
                if counterRS%4 == 2:
                    ...
                    m_pressed = bool(ctypes.windll.user32.GetAsyncKeyState(ord('M')) & 0x8000)
                    if m_pressed and not prev_m_pressed:
                        yolo_enabled = not yolo_enabled
                        status = "ON" if yolo_enabled else "OFF"
                        print(f"YOLO overlay toggled {status} (M key).")
                    prev_m_pressed = m_pressed

                    if newRealSenseRGB:
                        frame_rgb = realSenseCam.imageBufferRGB.copy()
                        if yolo_model is not None and yolo_enabled:
                            results = yolo_model(frame_rgb, conf=yolo_conf_threshold, iou=yolo_iou_threshold, verbose=False)[0]
                            detection_viz = frame_rgb.copy()
                            if results and results.boxes is not None:
                                for box in results.boxes:
                                    ...
                                    color = (0, 0, 255)
                                    cv2.rectangle(detection_viz,
                                                  (int(x1), int(y1)),
                                                  (int(x2), int(y2)),
                                                  color, 2)
                                    label = f'{yolo_model.names[int(box.cls)]}:{conf:.2f}'
                                    cv2.putText(detection_viz, label,
                                                (int(x1), int(max(y1 - 5, 15))),
                                                cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                                                color, 1, cv2.LINE_AA)
                            last_rgb_viz = detection_viz
                        else:
                            last_rgb_viz = frame_rgb
                    sending = probe.send(name="RealSense RGB Image",
                                         imageData=last_rgb_viz)
                if counterLidar%6 == 0:
