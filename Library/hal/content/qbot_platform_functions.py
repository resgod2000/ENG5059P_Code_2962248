...
class QBPMovement():
    ...
    def diff_drive_inverse_velocity_kinematics(self, forSpd, turnSpd):
        ...
        vL = forSpd - (turnSpd * self.WHEEL_BASE) / 2.0
        vR = forSpd + (turnSpd * self.WHEEL_BASE) / 2.0
...
class QBPVision():
    ...
    def image_find_objects(self, image, connectivity, minArea, maxArea):
        ...
        num_labels, labels, stats, centroids = cv2.connectedComponentsWithStats(
            image.astype(np.uint8), connectivity)

        for idx in range(1, num_labels):
            blob_area = stats[idx, cv2.CC_STAT_AREA]
            if minArea < blob_area < maxArea:
